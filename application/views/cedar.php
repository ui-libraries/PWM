...
<script type="text/javascript">
    ...
    function init() {
		//add a leaflet map 
//TODO: Consider make this page into a generic view and passing in the following line 
		...
		<?php
			for ($i=1;$i<$n;$i++){
				if (!empty($iconname))   { echo $iconname[$i]; }
				if (!empty($markername)) { echo $markername[$i]; }
				if (!empty($popupname))  { echo $popupname[$i]; }
 				if (!empty($bindpopup))  { echo $bindpopup[$i]; }
			}
		?>
